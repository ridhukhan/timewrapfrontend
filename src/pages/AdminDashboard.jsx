...
const AdminDashboard = () => {
  ...
  return (
    <div style={{ paddingTop: "60px" }} className="min-h-screen bg-zinc-950 text-white p-6">
      ...
      {orders.length === 0 ? (
        <p className="text-zinc-500 text-center mt-20 text-lg">No orders yet!</p>
      ) : (
            ...
            <tbody>
              {orders.map((order, i) => (
                ...
                >
                  ...
                  <td className="px-4 py-3 text-sky-400 font-semibold">{order.watchname}</td>
                  <td className="px-4 py-3">{order.quantity}</td>
                  <td className="px-4 py-3 text-green-400 font-bold">${order.price}</td>
                      ...
                      <button
                        ...
                        className={`px-3 py-1 rounded-lg text-xs font-bold transition-all duration-200 ${
                          order.status === "pending"
                            ...
                            : "bg-zinc-700 text-zinc-400 hover:bg-yellow-400 hover:text-black"
                        }`}
                      >
                        Pending
                      </button>
                      <button
                        ...
                        className={`px-3 py-1 rounded-lg text-xs font-bold transition-all duration-200 ${
                          order.status === "delivered"
                            ...
                            : "bg-zinc-700 text-zinc-400 hover:bg-green-500 hover:text-black"
                        }`}
                      >
                        Delivered
                      </button>
                      <button
                        ...
                        className={`px-3 py-1 rounded-lg text-xs font-bold transition-all duration-200 ${
                          order.status === "return"
                            ...
                            : "bg-zinc-700 text-zinc-400 hover:bg-red-500 hover:text-white"
                        }`}
                      >
                        Return
                      </button>
